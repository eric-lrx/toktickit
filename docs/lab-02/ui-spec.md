...
## 8. Visual inspection checklist (filled in at implementation time, tracked in `tests.md`)

- [ ] Colors match the token table exactly (checked with dev tools, not by eye)
- [ ] Editable vs. read-only fields are visually distinguishable at a glance
- [ ] Validation messages appear directly under their field, not only at the top
- [ ] Button hierarchy (primary/secondary/tertiary/destructive/disabled/busy) is consistent across screens
- [ ] No clipped labels or truncated buttons at any of the three breakpoints
- [ ] No overlapping elements at any of the three breakpoints
- [ ] No horizontal scrolling at the mobile breakpoint
- [ ] Screenshots captured under `artifacts/lab-02/screenshots/{create-ticket,my-tickets,ticket-detail}/` at desktop, tablet, and mobile widths
